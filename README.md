...
- **`strongestArgumentAgainst`** — required, every time, and **checked in code rather than trusted**. Four of the twenty recorded runs carrying this field published a pointer at the field itself — *"See strongestArgumentAgainst field above (duplicate not needed)"* — with the argument existing nowhere else in the report. Missing, not misfiled, on the one field whose job is to argue against the answer. A field that is only a pointer now triggers **one** more call asking for that field alone; if that also returns a pointer the field reads `NOT PRODUCED` and `honestLimits.noSteelman` travels with the report, rather than publishing a cross-reference that reads like content.
  The rule is *a pointer with nothing of substance after it*, not *a pointer and a short field*. The length version broke both ways: it passed a 308-character padded pointer and destroyed a 144-character genuine argument that opened by citing another section — the check deleting real evidence. A terse argument and a padded pointer are the same size, so length could never separate them. A pointer followed by enough filler still passes; that limit is named and pinned in `contract/conformance.json` rather than claimed closed.
...
- **`hypothesisVerdicts[].hypothesisNumber`** — which registered hypothesis a verdict adjudicates, so nothing has to be matched. `0` means the synthesis step wrote it after the evidence. `preRegisteredBy` says whether the stamp came from that number or was inferred from text, so a reader can tell a certainty from a guess. This replaced text matching because no lexical rule separates a compact rewording from a qualifier being stripped: genuine rewordings drop 8–12 content words, a subset attack drops 2.
  The number is **checked against the hypothesis it names**, not believed. It was authoritative with nothing examined for one release, which traded a stamp gameable by rewording for one gameable by typing a digit. The first cross-check then sat at 0.40, which separates only a different *subject* — and an audit showed the round-2 superset walking straight back in through the number path: a verdict swapping the registered scope for a different one (`"…only among part-time workers"` against a registered `"…modestly in the first two years"`) scored **0.545** and was stamped `preRegistered: true, preRegisteredBy: "hypothesisNumber"`. Certainty, for a claim the run never registered. Re-measured on every pair on record:
...
| **the bar** | **0.65** |
| scope substitution | 0.545 |
| superset attack | 0.455 |
| unrelated | 0.250 / 0.091 |

  **An ADDED negation is checked before any of that, because it is categorical and overlap is blind to it.** A registered *"…reduce teen employment"* against a post-hoc *"…do **not** reduce teen employment"* scores **1.000** — `not` is a stopword and `do` is two characters — so every threshold passes the opposite hypothesis. That is the third time a similarity measure here has been blind to a negation: the character bag scored *"output is stable"* against *"output is unstable"* at 1.000, `SequenceMatcher` at 0.941, and the token measure that replaced both scores a flat negation at 1.000. A flip is a different answer wearing the same words, so it gets its own check rather than a better number. The check is **symmetric, and runs in both the number path and the text path**. It fired one way for one release, on the reasoning that a positive rewording of a registered null should not be marked post-hoc — and that had the costs backwards. The accusation it avoided cost only the *label*: the verdict still stamped `preRegistered: true` through the text path at coverage 0.933. The direction it left open cost the *stamp*: deleting a registered negation while keeping every content word gives an **identical token set** — `not` is a stopword, `do` is two characters — so overlap is **1.000** and a registered null could be adjudicated as its exact opposite and stamped a prediction that survived. Fixing only the number path closed nothing, because the text path was blind by the same 1.000.

  The cost is taken knowingly: a positive restatement of a null (*"produce statistically similar fat loss"* for *"No meaningful difference"*) is marked post-hoc. Across every archived run only **8** verdict/registered pairs carry a number and **none** differ in negation, so this chooses between two unobserved failures — and the rule this project runs on is that a false *pre-registered* is the one to prevent while a false *post-hoc* only understates.

  **What the check still does not catch**, and the label now says so rather than reading as certainty: an intensity change (*"eliminate"* for *"reduce"*, 0.833) and a reversed causal direction (0.714) both pass. No lexical rule catches either — three rounds have established that — and an **antonym flip** carries no negator word at all, so *"raise teen employment"* against a registered *"reduce teen employment"* covers 0.833 and passes — a real polarity flip the check does not see. `preRegisteredBy` therefore reads `hypothesisNumber (subject, scope and added negation checked - NOT polarity, and not that the claim is identical)`, and all three uncatchable cases are pinned in `contract/conformance.json` as untagged named limits.
  **The negation check itself reversed five times**, and the fifth is the one that stuck: symmetric (accused the null) → ADD-only (opened the surgical-delete mirror) → symmetric again (re-accused the null, and gated BOTH stamp paths while its own docstring claimed label-only cost) → **direction-aware**: ADD fires categorically, DROP fires only on near-identity, because the two families separate on *character* similarity (surgical deletions 0.821–0.977, faithful compact rewordings 0.479–0.697) and inseparably on tokens (0.933 vs 1.000). The synonym-swap rewording ('not X' → 'rather than X', 0.855) sits inside the attack band, is marked post-hoc in both paths, and is pinned as the named limit — the understating direction, stated as the stamp it costs.
  ...
  **A terse verdict cannot be checked this way at all** — too few content words to score — so its number is believed. That hole is named rather than papered over, and `preRegisteredBy` is how you see which verdicts rest on an unchecked number.
  Terse hypotheses — fewer than four content words — are marked post-hoc rather than guessed at. Both builds used to fall back to character similarity there, and a negation is a tiny edit that inverts the meaning: *“output is stable”* against *“output is unstable”* scored 1.000 on one build's character bag and 0.941 on the other's `SequenceMatcher`. Across all 160 hypotheses in `runs/`, none is short enough to reach that path, so it protected nothing and risked the one failure the stamp exists to prevent.
- **`processCritique.untraceableCountMeans`** — the count is of distinct flagged *strings*, deduplicated on whitespace and case, not distinct problems. Two critics objecting to one sentence in different words count twice. Deduplicating by meaning needs a semantic judgement, and this codebase has twice measured that such judgements cannot be made with a similarity threshold.
- **A post-hoc *superset* of a registered hypothesis is not pre-registered either.** The match is measured against the verdict's own content, not `min(|A|,|B|)`, which a superset satisfies by construction. Audited: `"Minimum wage increases reduce employment"` extended with `", and the reduction persists for at least a decade"` was stamped `preRegistered: true`. It then came back a second time through `hypothesisNumber`, which was cross-checked too loosely to notice — see the table above. Both doors are shut; a superset now scores 0.455 and a scope substitution 0.545, against a bar of 0.65.
- **No verdict claims a hypothesis the contract never held.** Every entry in `hypothesisVerdicts` carries `preRegistered`. When framing produces nothing, the synthesis step can still invent hypotheses *after* seeing the evidence and adjudicate them — which is the opposite of what this tool sells — and the caveat beside the field used to assert the field was empty while it was not. Both are fixed: the verdicts are stamped, and `honestLimits.postHocHypotheses` says how many were written after the fact.
- **A recovered payload is never silently discarded.** `as_list` rescues `<item>`-wrapped arrays into strings; where the schema wants objects those were dropped, and because the corrective retry re-asks an identical question, all five attempts burned on a payload already in hand. Observed live: framing lost four hypotheses this way and the run degraded to an ordinary literature summary. The retry is now told what arrived and which keys were needed. It fires only when nothing survived — a bad item beside good ones is still just dropped.
- **`citationAudit.auditErrors`** — audit calls that returned nothing after retries. They are excluded from the accuracy denominator, which is correct, but they used to be excluded *silently*: under rate-limiting the "full verification pool" quietly shrank, so the headline number improved exactly when the run was degraded. `scope` now states the real denominator.
- **The citation audit re-fetches the page; it does not re-read the cache.** An audit served from the page cache re-reads the extractor's own artifact byte for byte, so the whole fetch-seam error class — a JS shell parsed as prose, a stale archive snapshot, an abstract standing in for the paper — reads back as perfect corroboration. The audit now goes to the network; if that read fails it falls back to the cached text and records `stats.auditRefetch.fellBackToCache` rather than calling the claim unreachable, because a transient 429 is not evidence about a claim.
- **An unread page is never reported as an unsupported claim.** When the engine knows it did not get the cited page — the fetch failed, the PDF was unreadable — the citation audit answers `unreachable` in code and spends no model call. When it got only the *abstract* because the publisher blocked it, the auditor is told so. Measured 2026-09-08 over 12 calls each: shown an empty page the auditor says `unreachable` 12/12, but shown an abstract it says **`unsupported` 12/12** — and `unsupported` is the only verdict that demotes a claim the panel already passed. Telling it what it holds moved that to `unreachable` 6/6 while leaving genuinely-unsupported and genuinely-supported claims untouched, against a control with zero self-noise.
- **An elided quote discloses what it skipped.** Fragments either side of a `...` are real page text, and what sits *between* them is where a limitation lives. A quote reading "the drug reduced mortality … the authors recommend approval" is 100% located while dropping "the effect vanished entirely in the over-65 subgroup". The panel is now told the skip length and instructed to weigh the omission, instead of being told "YES, 100% of it".
- **An ellipsis does not launder a stitched quote.** Fragments either side of a `...` must appear in source order and within a bounded distance. Audited 2026-09-15: before this, two sentences taken from different sections of a page and joined with an ellipsis scored `located-elided` at 100% — in either order — while the same two sentences without the ellipsis scored `partial`. The ellipsis was the entire difference between caught and certified.
- **A quote can be partly real.** Across the 46 published claims of two live runs, **97.8%** of quotes were located on the page they cite (45 of 46). The one that was not is a paper *title* quoted as if it were evidence — a genuine flag. Most of the rest are a genuine prefix with a diverging tail, not an invention — `quoteAudit.foundFraction` says which, and the panel is told before it votes. Before this, "VERBATIM" appeared in three prompts and in no code. **Correction, same day:** this was first published as 87–97%. That range was depressed by two bugs in the checker itself, not by the extractor — PDF line-break hyphenation (`con- clusive`) went unhandled, and `norm_quote` *mapped* the characters `webtext` *deletes* before the page ever reaches the model, so on any page carrying quotation marks a perfectly faithful quote scored `not-found` at 0.0. Re-scoring the same 46 quotes after both fixes gives 97.8%. The rate remains a lower bound: our PDF reader drops the `fi` ligature, so a page reads `signicant` and a faithful quote still scores short. Dropped characters inside a word cannot be normalised away without making the matcher too loose to mean anything.
...
- **The counter-evidence lens opts out of the junk filter.** That lens searches with the *claim* as the query and is looking for a source that contradicts it — and a contradiction routinely shares no vocabulary with the claim it refutes. Filtering on shared words dropped the whole result set, told the lens the web was silent, and logged the backend as poisoned.
- **`stats.searchHealth` names WHICH failure.** `challenged` (this IP is blocked — a proxy or SearXNG helps), `junk` (the engine answered with unrelated results — it is poisoned, not quiet), `fail` (actually down). All three used to read identically as `fail`, and they have three different fixes. The code already knew the difference and discarded it one layer up.
- **The junk filter guards every backend.** It used to guard one of eleven — and on a host without SearXNG, that one is the backend that is off, so the filter against a poisoned engine was protecting nothing at all.
