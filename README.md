...
- **`hypothesisVerdicts[].hypothesisNumber`** — which registered hypothesis a verdict adjudicates, so nothing has to be matched. `0` means the synthesis step wrote it after the evidence. `preRegisteredBy` says whether the stamp came from that number or was inferred from text, so a reader can tell a certainty from a guess. This replaced text matching because no lexical rule separates a compact rewording from a qualifier being stripped: genuine rewordings drop 8–12 content words, a subset attack drops 2.
  The number is **checked against the hypothesis it names**, not believed. It was authoritative with nothing examined for one release, which traded a stamp gameable by rewording for one gameable by typing a digit. The check is deliberately weak and only ever overrules on *subject*: measured on `runs/v10`, true rewordings score 0.82–1.00, a post-hoc superset 0.42–0.45 and an unrelated hypothesis at most 0.36, so the bar sits at 0.40. A verdict whose text has nothing to do with the hypothesis it numbers falls through to the labelled text path; a rewording that names the right number is still a certainty.
...
- **A post-hoc *superset* of a registered hypothesis is not pre-registered either.** The match is now measured against the verdict's own content, not `min(|A|,|B|)`, which a superset satisfies by construction. Audited: `"Minimum wage increases reduce employment"` extended with `", and the reduction persists for at least a decade"` was stamped `preRegistered: true`.
