...
| unrelated | 0.250 / 0.091 |
  ...
  **An ADDED negation is checked before any of that, because it is categorical and overlap is blind to it.** A registered *"…reduce teen employment"* against a post-hoc *"…do **not** reduce teen employment"* scores **1.000** — `not` is a stopword and `do` is two characters — so every threshold passes the opposite hypothesis. That is the third time a similarity measure here has been blind to a negation: the character bag scored *"output is stable"* against *"output is unstable"* at 1.000, `SequenceMatcher` at 0.941, and the token measure that replaced both scores a flat negation at 1.000. A flip is a different answer wearing the same words, so it gets its own check rather than a better number. The check fires **one way only** — a verdict adding a negation its hypothesis does not carry. The symmetric version falsely accused the commonest shape there is: **two of four** registered hypotheses in `runs/v10` carry a negator, H1 being the null hypothesis *"No meaningful difference…"*, so a faithful positive rewording of it was relabelled *"does not state that hypothesis"*. The reverse direction needs no check — a verdict asserting a difference covers 0.176–0.238 against a registered null, which the bar already catches.
